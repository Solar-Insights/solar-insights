...
export async function drawSolarInsightsChart(userSolarData: UserSolarData, costChart: HTMLElement) {
    const options = {
        ...
        series: makeTimeSeriesFromUserSolarData(userSolarData),
        grid: {
            show: false
        },
        xaxis: {
...
function makeTimeSeriesFromUserSolarData(userSolarData: UserSolarData) {
    const yearsList: Date[] = makeYearsList(userSolarData);
    const cumulativeCostWithSolar: [Date, number][] = makeCumulativeCostWithSolar(userSolarData, yearsList);
    const cumulativeCostWithoutSolar: [Date, number][] = makeCumulativeCostWithoutSolar(userSolarData, yearsList);
